...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>BEN EMOS Test Page</title>
	...
	<link href="css/ext-styles.css" rel="stylesheet" media="screen">
  ...
  <body>
    ...
    <div class="container">
        ...
        <div class="hero-unit">
            <div class="row">
                <div class="span6">
                <h2>EMOS Test Page</h2>
                <p>Go ahead, let your EMOS out</p>
                </div>
                <div class="span6">
			...
			echo ($select === "accounts") ? "<li class='active'>" : "<li>";
			echo "<a href='?layout=accounts\'>Accounts</a></li>";
			
			echo ($select === "brands") ? "<li class='active'>" : "<li>";
			echo "<a href='?layout=brands'>Brands</a></li>";
			
			echo ($select === "categories") ? "<li class='active'>" : "<li>";
			echo "<a href='?layout=categories'>Categories</a></li>";
			
			echo ($select === "clients") ? "<li class='active'>" : "<li>";
			echo "<a href='?layout=clients'>Clients</a></li>";
			
			echo ($select === "genres") ? "<li class='active'>" : "<li>";
			echo "<a href='?layout=genres'>Genres</a></li>";
			
			echo ($select === "opportunities") ? "<li class='active'>" : "<li>";
			echo "<a href='?layout=opportunities'>Opportunities</a></li>";
			
			echo ($select === "placements") ? "<li class='active'>" : "<li>";
			echo "<a href='?layout=placements'>Placements</a></li>";
			
			echo ($select === "vehicles") ? "<li class='active'>" : "<li>";
			echo "<a href='?layout=vehicles'>Vehicles</a></li>";
			...
			<form>
				<div class="input-append">
					<input type='hidden' name='layout' value=<?=$select?> />
					<input class="input-small" name="id" type="number" placeholder="ID #" >
...
function parseDateTime($date, $time) {
	//parse date
	$dateArray = array();
    $dateArray[] = strtok($date, "-");
    $body = '';

    for($i=0; $i<2; $i++) {
		$temp = strtok("-");
		$dateArray[] = (substr($temp, 0, 1) == "0") ? substr($temp, 1, 1) : $temp; //eliminates leading zeros
	}
	$body .= "$dateArray[1]/$dateArray[2]/$dateArray[0]";
	
	//parse time
	$timeArray = array();
	$isDefaultTime = true;
	$timeArray[] = strtok($time, ":-");
	for($i=0; $i<2; $i++) {
		$timeArray[] = strtok(":-");
	}
	foreach($timeArray as $timeCheck) {
		if($timeCheck != "00") {
			$isDefaultTime = false;
		}
	}
	...
	if($isDefaultTime) {
		$time = strtok("-");
		$timeArray[0] = (substr(($temp = strtok($time, ":")), 0, 1) == "0") ? substr($temp, 1, 1) : $temp;
		$timeArray[1] = strtok(":");
		$timeArray[2] = (($temp = strtok(":")) === FALSE) ? "00" : $temp;
		if(intval($timeArray[0]) > 12) {
			$meridiem = "PM";
			$timeArray[0] = intval($timeArray[0]) - 12; //changes from military time to 12 hour clock
		}
		$body .= " $timeArray[0]:$timeArray[1]:$timeArray[2] $meridiem";
	} else { 
		$timeArray[0] = (substr($timeArray[0], 0, 1) == "0") ? substring($timeArray[0], 1, 1) : $timeArray[0]; //eliminates leading zero
		if(intval($timeArray[0]) > 12) {
			$meridiem = "PM";
			$timeArray[0] = intval($timeArray[0]) - 12; //changes from military time to 12 hour clock
		}
